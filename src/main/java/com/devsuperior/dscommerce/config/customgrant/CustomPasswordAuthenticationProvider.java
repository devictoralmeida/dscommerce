...
public class CustomPasswordAuthenticationProvider implements AuthenticationProvider {
  ...
  public Authentication authenticate(Authentication authentication) throws AuthenticationException {
    ...
    this.username = customPasswordAuthenticationToken.getUsername();
    this.password = customPasswordAuthenticationToken.getPassword();

    UserDetails user = null;
    try {
      user = this.userDetailsService.loadUserByUsername(this.username);
    } catch (UsernameNotFoundException e) {
      throw new OAuth2AuthenticationException("Invalid credentials");
    }

    if (!this.passwordEncoder.matches(this.password, user.getPassword()) || !user.getUsername().equals(this.username)) {
      throw new OAuth2AuthenticationException("Invalid credentials");
    }

    this.authorizedScopes = user.getAuthorities().stream()
    ...
    CustomUserAuthorities customPasswordUser = new CustomUserAuthorities(this.username, user.getAuthorities());
